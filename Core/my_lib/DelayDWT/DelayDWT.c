...
void DWT_Delay_ms(uint32_t ms) { // millisecond
    uint32_t startTick  = DWT->CYCCNT,
             targetTick = DWT->CYCCNT + ms * (SystemCoreClock/1000);
 ...
 * Time is in microseconds (1/1000000th of a second), and time
 * is in millisecond (1/1000th).
 *
